...
const double MOVE_DISTANCE = 0.30;   // sisi segitiga 10 cm
...
void setup() {
  Serial.begin(115200);
  Serial.println("=== GLOBAL ODOMETRI TRIANGLE PATH ===");
  ...
  switch(state) {
    ...
    case MOVE_3: {
      ...
      if(distAcc >= MOVE_DISTANCE) {
        ...
        if(state == MOVE_1) state = TURN_1;
        else if(state == MOVE_2) state = TURN_2;
        else if(state == MOVE_3) state = TURN_3;
      }
      break;
    }
    ...
    case TURN_3: {
      ...
      // set sudut absolut tiap belokan
      if(state == TURN_1) targetAngle = 120;
      else if(state == TURN_2) targetAngle = 240;
      else targetAngle = 360; // 360° = 0°

      double dTheta = ((distR - distL) / wheelBaseMeters) * K_THETA * 180.0 / PI;
      theta_deg += dTheta;
      ...
      if(error > ANGLE_TOL) {
        ...
        if(state == TURN_1) state = MOVE_2;
        else if(state == TURN_2) state = MOVE_3;
        else state = FINISH;
      }
      break;
    }
    ...
    case FINISH:
      stopMotors();
      Serial.println("=== SELESAI SEGITIGA ===");
